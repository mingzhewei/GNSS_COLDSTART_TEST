...
运行：python gnss_coldstart_hmi.py（HMI 选文件）；或 python BY_coldstart.py <输出目录> [--split] <日志1> [日志2 ...]
"""
import os
import sys
import io
...
def make_images(data, out_dir, title_prefix=None):
    ...
    ax.set_ylabel('时刻（秒）')
    ax.set_title((title_prefix or '北云') + '冷启动关键时刻对比（t=0 为日志起点）', fontsize=13)
    ...
    ax.invert_yaxis()
    ax.set_xlabel('时间（秒）')
    ax.set_title((title_prefix or '北云') + '冷启动耗时分解（无时间 / 时间→单点 / 单点→浮点 / 浮点→固定）', fontsize=13)
...
def main():
    ...
    if len(args) < 2:
        print(__doc__)
        print('用法: python BY_coldstart.py <输出目录> [--split] <日志1> [日志2 ...]')
