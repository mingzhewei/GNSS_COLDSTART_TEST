# -*- coding: utf-8 -*-
"""
北云（UG016 组合导航接收机）GNSS 冷启动策略分析
=====================================================================
整合自项目原有脚本：analyze_0909.py / analyze_0909b.py / gen_report_0909.py
（原始日志解析核心与关键指标定义完全一致，此处为单文件整合版）

分析对象：data\\0909下午\\ 下 4 份 COM1 ASCII 日志
  20260909_163217_gnss_com1_indoor_01.log   室内冷启动 1
  20260909_164439_gnss_com1_outdoor_01.log 室外冷启动 1
  20260909_165759_gnss_com1_indoor_02.log   室内冷启动 2
  20260909_171057_gnss_com1_outdoor_02.log 室外冷启动 2

口径依据（北云科技《UG016 数据通信接口协议》，文本转录：by_manual/UG016.md）：
  * 报文          BESTGNSSPOSA（4.2.2，消息 ID 1429，输出最佳可用 GNSS 位置（无 INS））
  * 时标状态      报头字段 Time Status：UNKNOWN / COARSE / FINESTEERING，
                  UNKNOWN 表明尚未计算出准确的 GPS 时间
  * 解算状态      表 4-1：SOL_COMPUTED=完全解算；INSUFFICIENT_OBS=观测量不足；
                  VARIANCE=方差超过限值；COLD_START=冷启动尚未完全解算
  * 定位类型      表 4-2：SINGLE=单点解；NARROW_FLOAT=窄带浮点解；NARROW_INT=窄带固定解
  * 卫星字段      #SVs=跟踪卫星数，#solnSVs=解算卫星数，#solnMultiSVs=多频解算卫星数
  * 差分可用      Stn ID 非空且 != "0"（UG016 4.2.2 字段12：差分站台ID号，单点定位时ID为0）

输出（reports\\ 目录，每次运行覆盖生成）：
  report.html  自包含报告（HMI 可自动打开）
  report.md    同口径 Markdown 报告
  img_关键时刻.png / img_阶段耗时.png / img_卫星数量时间轴.png  报告插图
运行：python gnss_coldstart_hmi.py（HMI 选文件）；或 python BY_coldstart.py <输出目录> <日志1> [日志2 ...]
"""
import os
import sys
import io
...
TAG = '北云GNSS冷启动分析报告'
MANUAL = '北云科技《UG016 数据通信接口协议》'

if sys.stdout.encoding and sys.stdout.encoding.lower() not in ('utf-8', 'utf8'):
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

# 定位类型分组（UG016 表 4-2）：1=单点类 / 2=浮点类 / 3=固定类，其余归 0=无解
CAT1 = {'SINGLE', 'PSRDIFF', 'WAAS', 'INS_PSRSP', 'INS_PSRDIFF', 'INS_SBAS'}
CAT2 = {'L1_FLOAT', 'IONOFREE_FLOAT', 'NARROW_FLOAT', 'FLOATCONV', 'WIDELANE',
        'NARROWLANE', 'INS_RTKFLOAT'}
CAT3 = {'L1_INT', 'WIDE_INT', 'NARROW_INT', 'INS_RTKFIXED', 'RTK_DIRECT_INS',
        'PPP', 'INS_PPP'}
TS_CODE = {'UNKNOWN': 0, 'COARSE': 1, 'FINESTEERING': 2}
SOL_ORDER = ['INSUFFICIENT_OBS', 'VARIANCE', 'SOL_COMPUTED']
PT_CN = {'NONE': '无解', 'SINGLE': '单点', 'NARROW_FLOAT': 'RTK浮点', 'NARROW_INT': 'RTK固定'}
...
def make_images(data, out_dir):
    ...
    ax.set_ylabel('时刻（秒）')
    ax.set_title('北云冷启动关键时刻对比（t=0 为日志起点）', fontsize=13)
    ...
    ax.invert_yaxis()
    ax.set_xlabel('时间（秒）')
    ax.set_title('北云冷启动耗时分解（无时间 / 时间→单点 / 单点→浮点 / 浮点→固定）', fontsize=13)
...
<head>
<meta charset="UTF-8">
<title>北云GNSS冷启动分析报告（室内/室外）</title>
<style>__CSS__</style>
</head>
<body><div class="wrap">
<h1>北云（UG016）GNSS 冷启动策略分析（室内 / 室外冷启动）</h1>
...
<div class="note">口径（UG016）：报文按行做 CRC 校验（"#" 开头为 32 位 CRC，"$" 开头为 NMEA 异或）；$KSXT 为自定义 8 位 CRC32 私有报文，按 NMEA 异或校验判失败属正常、非故障。卫星维度口径：#SVs=跟踪卫星数、#solnSVs=参与解算卫星数、#solnMultiSVs=参与解算的多频卫星数（UG016 4.2.2 字段 15/16/18）；若需逐星载噪比（C/N0）与分星座可见星，需另行录制 GSV 或 RANGEA 报文（如 <span class="mono">log com1 gsv ontime 1</span>）。</div></div>
...
def build_outputs(data, imgs, src_desc=None):
    ...
    # ---------- Markdown ----------
    md = []
    md.append('# 北云（UG016）GNSS 冷启动策略分析（室内 / 室外冷启动）')
    md.append('')
    ...
    md.append('> 口径（UG016）：报文按行做 CRC 校验（`#` 开头为 32 位 CRC，`$` 开头为 NMEA 异或）；'
              '`$KSXT` 为自定义 8 位 CRC32 私有报文，按 NMEA 异或校验判失败属正常、非故障。'
              '卫星维度口径：#SVs=跟踪卫星数、#solnSVs=参与解算卫星数、#solnMultiSVs=参与解算的多频卫星数。')
    md.append('')
    ...
    md.append('|---|---|---|---|')
    md.append('| 获得卫星时间 | BESTGNSSPOSA 报头 Time Status 首次脱离 UNKNOWN | 报头字段 6 | UNKNOWN 表明尚未计算出准确的 GPS 时间 |')
    md.append('| COARSE→FINE | Time Status 首次 COARSE / FINESTEERING | 报头字段 6 | UNKNOWN / COARSE / FINESTEERING 三态 |')
    md.append('| 首单点 / 首浮点 / 首固定 | 位置类型 Pos Type 首次 SINGLE / NARROW_FLOAT / NARROW_INT | 表 4-2 | SINGLE=16 单点解；NARROW_FLOAT=34 窄带浮点解；NARROW_INT=50 窄带固定解 |')
    md.append('| 差分改正可用 | Stn ID 非空且 != "0" | 4.2.2 字段 12 | 差分站台 ID 号；单点定位时 ID 为 0；基站 ID 随数据动态变化 |')
    md.append('| 无解段解算状态 | Sol Type = INSUFFICIENT_OBS / VARIANCE | 表 4-1 | 观测量不足 / 方差超过限值；SOL_COMPUTED=完全解算；COLD_START=冷启动尚未完全解算 |')
    md.append('| 跟踪星 / 解算星 / 多频解算 | #SVs / #solnSVs / #solnMultiSVs | 4.2.2 字段 15/16/18 | 跟踪卫星数 / 解算卫星数 / 多频解算卫星数 |')
    md.append('')
